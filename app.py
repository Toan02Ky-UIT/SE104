...
def add_to_cart(product_id):
    ...
    if size.stock == 0:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify(success=False, message="Size này hết hàng.")
    ...
    if existing_item:
        if existing_item.quantity < size.stock:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify(success=False, message="Số lượng sản phẩm trong kho không đủ.")
...
def admin():
    ...
    return render_template('admin.html', products=products, orders=orders, users=users, revenue_data=revenue_data, report_type=report_type)
...
def delete_product(product_id):
    ...
    if order_details:
        flash("Không thể xóa sản phẩm vì có đơn hàng liên quan.", "danger")
        return redirect(url_for('admin'))
    product = Product.query.get_or_404(product_id)
...
def update_order_status(order_id):
    ...
    if new_status == "Đã hủy":
        ...
        current_rank = status_order.get(order.status, 0)
        new_rank = status_order.get(new_status, 0)

        if new_rank < current_rank:
            flash("Không được chuyển đơn về trạng thái trước đó.", "danger")
            return redirect(url_for('admin' if session.get('role') == 'admin' else 'staff'))
...
def staff():
    ...
    products = Product.query.all()
    orders = Order.query.all()
    return render_template('staff.html', products=products, orders = orders)
